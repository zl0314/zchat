    ...
    protected function initDefaultButtons()
    {
        if (! isset($this->buttons['view'])) {
            $this->buttons['view'] = function ($url, $model, $key, $index, $gridView) {
                return Html::a('<i class="layui-icon"></i> ' . Yii::t('yii', 'View'), $url, [
                    'title' => Yii::t('app', 'View'),
                    'data-pjax' => '0',
                    'class' => 'btn layui-btn layui-btn-small',
                ]);
            };
        }
        if (! isset($this->buttons['view-layer'])) {
            $this->buttons['view-layer'] = function ($url, $model, $key, $index, $gridView) {
                ...
                return Html::a('<i class="layui-icon"></i> ' . Yii::t('yii', 'View'), 'javascript:void(0)', [
        ...
        if (! isset($this->buttons['update'])) {
            $this->buttons['update'] = function ($url, $model, $key, $index, $gridView) {
                return Html::a('<i class="layui-icon"></i> ' . Yii::t('app', 'Update'), $url, [
